...
def estimate_triangular_y(m, Q, s, n):
    y = sp.symbols('y')
    Area = m*y**2
    P = 2*y*np.sqrt(1+m**2)
    R = Area / P
    f_Q = R**(2/3) * (s**0.5) / n * Area
    equation = sp.Eq(f_Q, Q)
    solutions = sp.nsolve(equation, y, 1)
    return solutions


def estimate_parameters(m, Q, s, y):
    Area = m*y**2
    V = Q /Area
    P = 2*y*np.sqrt(1+m**2)
    R = Area / P
...
def plot_triangular(m, width, h, y):
    # find low angle
    ...
    # plot water body
    wb_1 = [0,0]
    wb_left = [y*np.tan(suppl_theta_low), y]
    wb_right = [- y*np.tan(suppl_theta_low), y]

    points_water_body = [wb_1, wb_left, wb_right]

    p_right = Polygon(points_all_right, facecolor = 'gray')
    p_left =  Polygon(points_all_left, facecolor = 'gray')
...
with col2:
    st.write(f" **Triangular**")
    st.image('final_images/triangular-cross-section.png', width=400, caption='Triangular cross section')
    m = st.text_input(label="m (-)", value="1.50", help='m is the slope of the line (change in y:1/change in x)')
    ...
    with col3:
        st.write(f" **Outputs**")
        y_rect = estimate_triangular_y(m, Q, s, n)
        rounded_y_rect = round(y_rect, 3)

        # paramters claculation
        Area, velocity, wetted_perimeter, hydraulic_radius =  estimate_parameters(m, Q, s, y_rect)
        ...
        thickness = 0.2
        plot_triangular(m, thickness, h, y_rect)
        st.image('Final_plot.png', width=400, caption='Estimated Triangular cross section')
        ...
        # call report - word
        prod_report = Report(type='triangular', params_input = input_params, params_output = est_params)
...
#### Fix problem with checking water -deplth compared to height
